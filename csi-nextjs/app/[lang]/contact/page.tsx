...
function ContactForm({ lang }: { lang: Lang }) {
  ...
  const [form, setForm] = useState({
    first_name: '', last_name: '', city: '', email: '', subject: '', comments: '',
  });
    ...
    <>
      ...
      <div className="section">
        <div className="container">
          ...
          <div className="notice" style={{ marginTop: 12 }}>{t(lang, 'contact.notice')}</div>

          {status === 'success' ? (
            <div className="notice" style={{ marginTop: 16 }}>{t(lang, 'contact.success')}</div>
          ) : (
